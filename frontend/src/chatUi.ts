import { CommChatMessage } from "../../common/commonTypes";
...
export default class ChatUi {
  ...
  constructor(private networkManager: NetworkManager) {
    ...
    this.chatContentContainer = document.querySelector("#chatContentContainer");

    this.addLineToChat({
      text:
        "Welcome to Dynamite Dan Multiplayer chatroom complex!<br/>" +
  ...
  private addInputToChat(focus = false, oldValue: string) {
    ...
    this.chatContentContainer.appendChild(input);
    this.chatContainer.scrollTop = this.chatContainer.scrollHeight;
    focus && input.focus();
  }

  addLineToChat(message: CommChatMessage) {
    const text = message.text;
    const specialStyle = message.special;
    ...
    this.chatContentContainer.appendChild(line);
